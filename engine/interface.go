...
func (iface *Interface) QPrepare() {
	...
	for i := 0; i < 3; i++ {
		which := portList[i]
		port := utils.GetProperty(node, which).(map[string]portType.PortFeature) // get value by property name
		...
		for name, config := range port {
			...
			if feature == portType.TypeTrigger {
				...
				panic("Port type or feature for initialization was not recognized")
			}
			...
			utils.SetProperty(iface, which, linkedPort.CreateLinker())
		}
	}
}
